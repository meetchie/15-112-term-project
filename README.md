...
Light is a platformer with three freely accessible levels: an underwater cave system on the bottom, caves in the middle, and trees on top. The player traverses the map collecting fireflies. Every once in a while, the screen flashes and an event occurs — the player must seek shelter in the light spot before the flashing ends. The game ends when the player finds all the fireflies, or dies to the spiders during an event.
